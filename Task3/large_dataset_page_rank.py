...
# persist the links DataFrame as in-memory objects
links = spark.read.csv(input_file, comment="#", sep=r'\t').toDF("Page", "Neighbor").select(lower(col('Page')).alias('Page'), lower(col('Neighbor')).alias('Neighbor')).filter("(Neighbor NOT LIKE '%:%' OR Neighbor LIKE '%category:%') AND (Page NOT LIKE '%:%' OR Page LIKE '%category:%')").groupBy("Page").agg(collect_list("Neighbor")).persist()
